...
**Nota sobre validação local**: o ambiente desta sessão não conseguiu completar um `next build` de produção para o `platform/` — falha com `SIGBUS` no processo de build, uma limitação do sistema de ficheiros montado (o mesmo tipo de limitação já registada durante o trabalho no `website/`: operações de escrita/remoção em massa em pastas sincronizadas falham de forma inconsistente, sem ser um problema do código). Em vez disso, a validação feita foi: `prisma generate` (concluído com sucesso, cliente Prisma gerado a partir do schema atualizado) e `tsc --noEmit` sobre todo o projeto (zero erros). O ambiente de build da Vercel/CI, ao correr num sistema de ficheiros normal, não deverá ter esta limitação — mas o primeiro `next build` real (na Vercel ou localmente fora deste sandbox) deve ser tratado como o primeiro teste ainda por fazer antes de considerar esta funcionalidade "verificada em produção".
...
| Portal do Cliente (`platform/`) | Código completo e validado (`tsc` limpo) — falta Postgres + hosting de produção |
